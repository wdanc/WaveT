...
from dwt.dwt_utils import reflect
...
class DWT1DForwardwd(nn.Module):
    ...
    def __init__(self, J=1, wave='db1', mode='zero'):
        ...
        self.h0 = nn.Parameter(filts[0])
        self.h1 = nn.Parameter(filts[1])
        self.J = J
        self.mode = mode


    def apply(self, x, h0, h1, mode):
        mode = int_to_mode(mode)
        x = x[:, :, None, :]
        h0 = h0[:, :, None, :]
        h1 = h1[:, :, None, :]
    ...
    def forward(self, x):
        ...
        for j in range(self.J):
            x0, x1 = self.apply(x0, self.h0, self.h1, mode)
            highs.append(x1)
